...
def main():
    TASK = "wo_knowledge_node_TableClassification"
    ...
    print("=" * 70)
    print("Knowledge Tree Table Classification Evaluation")
    print("=" * 70)
    ...
    for idx, row in tqdm(test_data.iterrows(), total=len(test_data)):
        ...
        _, _, final_pred, _ = kTree_paper_classification(model, tokenizer, title, abstract)
        if final_pred == "Marine_Pb":
            for i, table in enumerate(table_data):
                ...
                _, prompt = generate_ablation_prompt(task=TASK,title=title,abstract=abstract,table_content=table_content)
